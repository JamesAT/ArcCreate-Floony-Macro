...
MacroNew(
    ...
    function(cmd)
        ...
        -- Check if the timing group with such name already exists, so it uses that tg and not overloaded with lots of it
...
-- // STASH TIMING // --
    -- / I just copy paste this from camera stash and modify it to only pick timing, i love recycling
...
-- // SCRAPPED MACRO // --
--[[ not sure if I have any uses for this ngl.
...
MacroNewNOCMD(
    ...
    function()
        ...
        -- Set a limit just in case someone input 100, and I don't want that to happened, they can just modify this tho.
        ...
        for trailIndex = 1, trailCount - 1 do
            ...
            local alpha255 = (1.0 - (trailIndex / trailCount)) * 255
            ...
            -- Copy Notes, and then move it gradually
            ...
            -- Handle Arctaps (i still HATE this)
            for _, note in ipairs(notes) do
                if note.is('arctap') then
...
local function createArcsForNotes(cmd, selectedJudgeable, color)
    local sortableList = {}
    for i = 1, #selectedJudgeable do
        local note = selectedJudgeable[i]
    ...
    for _, wrapper in ipairs(sortableList) do
        local note = wrapper.obj
        local position = nil
    ...
    -- / Macro
local function runPlacementMacro(cmd, startColor)
    local selectedJudgeable = table.icollapse(
        Event.getCurrentSelection(EventSelectionConstraint.create().any()), 
        'tap', 'hold', 'arctap'
    )
    ...
    if #selectedJudgeable < 2 then
        notify("Please select at least two notes.")
        return false
    end

    if not firstExecute then
        dialogNotifyCustom('WARNING', 'Placement cues will be generated in a "Stream_TEMP" timing group.\nProceed?')
        firstExecute = true
    end

    createArcsForNotes(cmd, selectedJudgeable, startColor)
end

MacroNew('floony.placement', 'bluefirst', "Left Hand First", "eb59", function(cmd) runPlacementMacro(cmd, 0) end)
MacroNew('floony.placement', 'redfirst', "Right Hand First", "eb52", function(cmd) runPlacementMacro(cmd, 1) end)
    ...
    function(cmd)
        local dialogContent = {
            ...
            DialogField.create("placement")
                .description(generateDialogField("floony.placement", "Stream Placement", "Generate 1ms colored arcs to indicate finger placement (Blue=Left Red=Right)")),
